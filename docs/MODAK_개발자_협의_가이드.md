...
```
모든 계정 = 주영님 호주 법인 명의로 가입
개발자는 팀원으로 초대하는 방식
소스코드 소유권 = 주영님 (MODAK Group Pty Ltd)
```
...
> 모든 계정은 주영님 법인 명의로 가입 후 개발자 초대

| 서비스 | 개발자 권한 | 가입 주체 |
|--------|-----------|---------|
| GitHub | Admin | 주영님 계정 |
| Firebase | Editor | 주영님 법인 계정 |
| Supabase | Admin | 주영님 법인 계정 |
| AWS | IAM 계정 생성 후 전달 | 주영님 법인 계정 |
| Google Cloud Console | Editor | 주영님 법인 계정 |
| Apple Developer | Admin | 주영님 법인 계정 |
| Google Play | Release Manager | 주영님 법인 계정 |
| Stripe | Developer | 주영님 법인 계정 |
...
```
① 소스코드 소유권 — 주영님 (MODAK Group Pty Ltd)
② GitHub 레포 — 주영님 계정 기준으로 관리
③ 계약서 작성 가능한지
④ 선금 / 중도금 / 잔금 비율 (일반적으로 30 / 40 / 30)
⑤ 런칭 후 무상 버그 수정 기간
⑥ 장기 유지보수 단가 기준
```
...
견적서 잘 받았습니다. 몇 가지 추가 협의 사항이 있어요.
...
2. 카카오 로그인 추가
   → 추가 비용/일정 + 구현 경험 있으신지요
   → 사업자 관련 사항은 별도 안내 드릴게요

3. 관리자 페이지 Supabase 연동
   → UI 완성본 있습니다 (HTML 파일 전달 가능)
   → 앱과 같은 DB 공유 필요
   → 관리자 권한 체계 포함
   → 추가 비용/일정 어떻게 되는지요
...
- AWS ECS 월 운영 비용 예상
- Supabase 월 비용 예상
- 소스코드 소유권 주영님 계정으로 관리 가능한지
- 런칭 후 무상 버그 수정 기간
- 계약서 작성 가능한지
- 선금/중도금/잔금 비율

[진행 방식]
- 모든 계정은 제 법인 명의로 가입 후 초대 방식으로 진행할게요
- 2주마다 중간 데모 확인 원합니다
